...
Este relatório apresenta a construção de um gráfico animado desenvolvido em R utilizando os pacotes ggplot2 e gganimate. A animação permite visualizar a variação da temperatura global ao longo dos meses para diferentes anos, tornando a análise mais acessível e intuitiva. Para garantir maior usabilidade e acessibilidade, foi adotada uma paleta de cores adaptada para daltônicos, seguindo diretrizes de visualização inclusiva. O objetivo é que o gráfico possa ser interpretado corretamente por um público mais amplo, incluindo pessoas com diferentes tipos de deficiência visual relacionada à percepção de cores. As principais instruções para o uso são:

-   O conjunto de dados deve conter colunas para ano, mês e temperatura.
-   A variável mês deve estar no formato fator ordenado para garantir a correta disposição dos dados.
...
Os dados utilizados nesta animação foram obtidos a partir do site da NASA, na seção "Tables of Global and Hemispheric Monthly Means and Zonal Annual Means". Esse conjunto de dados apresenta a variação de temperatura global, do hemisfério norte, hemisfério sul e por zonas. O primeiro, global, é o que nos interessa. Os dados são disponibilizados no formato .csv e contêm informações mensais sobre anomalias de temperatura em relação à média histórica. Antes da construção do gráfico, realizamos alguns procedimentos essenciais para garantir a integridade e a correta estruturação dos dados: - Remoção de linhas iniciais não estruturadas: O arquivo contém metadados e informações explicativas nas primeiras linhas, que foram descartadas durante a importação. - Tratamento de valores ausentes: Alguns registros utilizam \*\*\* para representar dados faltantes. Esses valores foram convertidos para NA, evitando distorções na visualização.
...
A animação gerada evidencia a evolução das anomalias de temperatura ao longo do tempo, mostrando um aumento gradual nos anos mais recentes e reforçando a tendência de aquecimento global. A disposição radial facilita a identificação de padrões sazonais e a comparação entre diferentes períodos, destacando tendências de longo prazo e variações sazonais.

A seguir, apresentamos um exemplo do gráfico animado gerado com os dados fornecidos.

![Gráfico Animado](espiral.gif)
...
-   **Configuração adequada da escala temporal e espacial**: Essa configuração melhora a leitura dos dados e evita distorções na representação gráfica.
...
O gráfico animado desenvolvido permite uma visualização clara e interativa da variação da temperatura global ao longo dos anos. A implementação desse tipo de visualização facilita a compreensão das mudanças climáticas ao longo do tempo e destaca padrões sazonais e tendências globais. Além disso, a inclusão de uma paleta de cores acessível para daltônicos reforça a importância da acessibilidade na comunicação visual de dados. Apesar dos desafios enfrentados, como a complexidade na conexão dos anos e o tempo de renderização, o resultado final foi satisfatório. O projeto ainda possui espaço para melhorias, como otimizações na performance e a criação de interfaces interativas que possibilitem maior exploração dos dados. Essas melhorias futuras podem tornar o gráfico ainda mais eficiente para a análise climática e a comunicação científica.

# Referências

[@article; @gganimate; @ggplot2; @monge-rodrC-guez2022a; @nasa; @nasa_gistemp; @TheClimateSpiralDemonstratesthePowerofSharingCreativeIdeas; @abnt]
